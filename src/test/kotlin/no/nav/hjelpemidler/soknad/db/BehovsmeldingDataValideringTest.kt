...
internal class BehovsmeldingDataValideringTest {
    ...
    fun `Validering av en normal søknad i dagens format`() {
        ...
        val filtrertRaw = JacksonMapper.objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(filtrert)

        assertNull(filtrert.bestillingsordningsjekk, "Forventet at bestillingsordningssjekk ble nullet ut før det sendes til kommunen")
        assertNull(filtrert.soknad.innsender, "Forventet at søknad->innsender ble nullet ut før det sendes til kommunen")
        assertFalse(filtrertRaw.contains("bestillingsordningsjekk"))
        assertFalse(filtrertRaw.contains("innsender"))
    }
}
