...
class Database(private val dataSource: DataSource, private val clock: Clock) :
    Transaction,
    Closeable {
    private val slackClient = slack(engine = Apache5.create())

    override suspend operator fun <T> invoke(returnGeneratedKeys: Boolean, block: suspend StoreProvider.() -> T): T = transaction(dataSource, strict = true, returnGeneratedKeys = returnGeneratedKeys) {
